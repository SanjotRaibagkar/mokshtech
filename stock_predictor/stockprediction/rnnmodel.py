...
class ml_dpmodels(object):
    ...
    def build_predictmodel(self,dataframe,skipdays):
        ...
        self.report_dict[header] = pd.DataFrame(columns=['symbol','Days','RSI','BBANDS','MA1','MA2','MA3','MA4','MSE','RMSE','Actual','Forcasted'])
        ...
        self.report_dict[header].set_value(header,'MA1',rep_header[7][2:])
        self.report_dict[header].set_value(header,'MA2',rep_header[8][2:])
        self.report_dict[header].set_value(header,'MA3',rep_header[9][2:])
        if len(rep_header)>10:
            self.report_dict[header].set_value(header, 'MA4', rep_header[10][2:])
        else:
